...
The [full review](http://f1000research.com/articles/3-62/v1#article-reports), with overarching comments preceding the link to the detailed annotations, is available form the [article's onlne version](http://f1000research.com/articles/3-62/) at F1000 Research. You will notice that the review includes a complete transcript of the online annotations; see below for why.
...
* Comments can be edited or deleted later, such as if later parts of the manuscript put the commented text in a different light. Comments can also be tagged, which could be used to distinguish between major and minor comments (but I didn't).
...
1. Although the comments comprising a review can be [retrieved in whole through a URL reference][comments URL], bundling them up in the form needed remains lacking.
    * The Hypothes.is platform doesn't yet provide a URL interface for showing annotations on top of the online text that was annotated[^Diigo link feature]. This view requires manually [installing a browser extension](http://hypothes.is/alpha), turning it on when on the article page, and activating highlighting mode. 
